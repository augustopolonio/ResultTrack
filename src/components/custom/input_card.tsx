...
export default function InputCard( { title, description, imageSrc }: InputCardProps ) {
  ...
  <Play className="ml-auto" /> {/* Add ml-auto to push Play icon to the end */}
    ...
    <DrawerContent className="flex flex-col items-center">
            ...
            <DrawerHeader>
              {time === 0 ? <h1 className="text-3xl">Finalizado!</h1> :
              <DrawerTitle className="text-6xl">
                {Math.floor(time / 60).toString().padStart(2, '0')}:{(time % 60).toString().padStart(2, '0')}
              </DrawerTitle>
              }
              <DrawerDescription>{time === 0 ? 'De 0 a 5, como foi sua experiência?' : '' }</DrawerDescription>
            </DrawerHeader>
            <DrawerFooter className="pb-16">
          
            {isTimerRunning ? '' : (
              <Button className="w-32 h-14" onClick={startTimer}>Iniciar</Button>
            )}

            {time > 0 ? '' : (
              <div className="flex flex-col gap-6 items-center">
                <Input placeholder="Nota" className="w-20 h-10" type="number" max={5}/>
                <DrawerClose asChild>
                  <Button className="w-32 h-14" onClick={onSaveNote}>Salvar</Button>
                </DrawerClose>
              </div>
            )}
          </DrawerFooter>
